...
def register_booking_handlers(bot):
    ...
    def process_booking_step(message: types.Message):
        ...
        if message.text.startswith('/') or message.text in ['📖 Меню', '🎁 Карта лояльности', '🤝 Привести товарища', '🗣 Спроси у Евгенича', '🥃 Получить настойку по талону', '📍 Забронировать стол', '👑 Админка', '📨 Отправить БРОНЬ']:
            return
