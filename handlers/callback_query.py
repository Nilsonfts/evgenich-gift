...
def register_callback_handlers(bot, scheduler, send_friend_bonus_func, request_feedback_func):
        ...
        try:
            ...
            if chat_member.status in ['member', 'administrator', 'creator']:
                try:
                    ...
                    logging.warning(f"Не удалось удалить сообщение при проверке подписки: {e}")
    ...
    def handle_redeem_reward(call: types.CallbackQuery):
        ...
        if database.update_status(user_id, 'redeemed'):
            try:
                ...
                logging.warning(f"Не удалось удалить сообщение при погашении купона: {e}")
            ...
            if referrer_id:
                ...
                scheduler.add_job(send_friend_bonus_func, 'date', run_date=run_date, args=[referrer_id, friend_name])
                logging.info(f"Запланирована отправка бонуса рефереру {referrer_id} на {run_date}.")

            feedback_run_date = datetime.now() + timedelta(hours=24)
            scheduler.add_job(request_feedback_func, 'date', run_date=feedback_run_date, args=[user_id])
            logging.info(f"Запланирован запрос ОС пользователю {user_id} на {feedback_run_date}.")
    ...
    def callback_main_menu_choice(call: types.CallbackQuery):
        ...
        try:
            bot.edit_message_text(
                texts.MENU_PROMPT_TEXT, call.message.chat.id, call.message.message_id,
    ...
    def callback_menu_nastoiki_main(call: types.CallbackQuery):
        ...
        try:
            bot.edit_message_text(
                texts.NASTOIKI_MENU_HEADER, call.message.chat.id, call.message.message_id, 
                reply_markup=keyboards.get_nastoiki_categories_keyboard(), parse_mode="Markdown"
            )
        except ApiTelegramException as e:
             logging.warning(f"Не удалось отредактировать сообщение меню: {e}")
    ...
    def callback_menu_category(call: types.CallbackQuery):
        ...
        try:
            bot.edit_message_text(
                ...
                reply_markup=keyboards.get_nastoiki_items_keyboard(), parse_mode="Markdown"
            )
        except ApiTelegramException as e:
            logging.warning(f"Не удалось отредактировать сообщение категории: {e}")
    ...
    def callback_menu_food_main(call: types.CallbackQuery):
        ...
        try:
            bot.edit_message_text(
                texts.FOOD_MENU_HEADER, call.message.chat.id, call.message.message_id, 
                reply_markup=keyboards.get_food_categories_keyboard(), parse_mode="Markdown"
            )
        except ApiTelegramException as e:
            logging.warning(f"Не удалось отредактировать сообщение меню еды: {e}")
    ...
    def callback_food_category(call: types.CallbackQuery):
        ...
        try:
            bot.edit_message_text(
                text, call.message.chat.id, call.message.message_id, 
                reply_markup=keyboards.get_food_items_keyboard(), parse_mode="Markdown"
            )
        except ApiTelegramException as e:
            logging.warning(f"Не удалось отредактировать сообщение категории еды: {e}")
