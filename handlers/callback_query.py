...
def register_callback_handlers(bot, scheduler, send_friend_bonus_func, request_feedback_func):
    ...
    @bot.callback_query_handler(func=lambda call: not (call.data.startswith('admin_') or call.data.startswith('boss_') or call.data.startswith('booking_') or call.data.startswith('source_') or call.data.startswith('bar_') or call.data.startswith('broadcast_') or call.data.startswith('newsletter_click_') or call.data.startswith('city_select_') or call.data.startswith('review_star_') or call.data in ['confirm_booking', 'cancel_booking', 'confirm_saved_contact', 'change_contact']))
