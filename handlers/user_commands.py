...
def register_user_command_handlers(bot):
    ...
    def handle_loyalty_card(message: types.Message):
        ...
        # ── Текст карты ──
        # Баланс/уровень показываем В САМОМ Mini App (там официальные данные из GMB).
        # В боте — короткий информационный экран с условиями программы и кнопка
        # «Открыть карту» (Mini App открывается прямо в Telegram).
        greet_name = full_name or "Товарищ"

        loyalty_text = (
            f"🎁 <b>Карта лояльности «Евгенич»</b>\n\n"
            f"Привет, {greet_name}!\n\n"
            f"<b>Условия программы:</b>\n"
            f"  🤝 <b>ТОВАРИЩ</b> — 5% кэшбэка (стартовый)\n"
            f"  🎩 <b>ИНТЕЛЛИГЕНТ</b> — 10% при сумме покупок от 40 000 ₽\n"
            f"  💎 <b>БУРЖУЙ</b> — 15% при сумме покупок от 100 000 ₽\n\n"
            f"💰 Бонусами можно оплатить до <b>30%</b> от чека\n"
            f"🎂 В день рождения — тебя ждёт <b>подарок</b> 🎁\n\n"
            f"👇 Жми кнопку — внутри регистрация и личный кабинет (баланс, история, QR для бармена):"
        )
