...
def register_admin_handlers(bot):
    ...
    # Исключаем callbacks, которые обрабатывает newsletter_manager/newsletter_buttons
    _newsletter_prefixes = ('admin_newsletter_type_', 'admin_newsletter_test_', 'admin_newsletter_send_',
                            ...
                            'admin_button_')
    @bot.callback_query_handler(func=lambda call: (call.data.startswith('admin_') or call.data.startswith('boss_')) and not call.data.startswith(_newsletter_prefixes))
    ...
    @bot.message_handler(commands=['force_restart'])
    def handle_force_restart_command(message: types.Message):
        if not is_admin(message.from_user.id):
            return
        
        try:
            bot.reply_to(message, "🔄 Принудительный перезапуск бота...\nБот будет перезапущен через 3 секунды.")
            ...
            def restart_bot():
                try:
                    time.sleep(3)
                    logging.info("🔄 ФОРСИРОВАННЫЙ ПЕРЕЗАПУСК БОТА по команде админа")
                    os._exit(0)  # Принудительный выход для перезапуска Railway контейнера
                except Exception as e:
                    logging.error(f"Ошибка при перезапуске: {e}")
                    os._exit(1)  # Аварийный выход
            
            threading.Thread(target=restart_bot, daemon=True).start()
        except Exception as e:
            logging.error(f"Ошибка обработки команды /force_restart: {e}")
            bot.reply_to(message, f"❌ Ошибка при перезапуске: {e}")
