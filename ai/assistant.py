...
def create_system_prompt(updates_string: str, user_concept: str = "evgenich", is_group_chat: bool = False) -> str:
    ...
    base_prompt = (
        ...
        f"💳 ВАЖНО ПРО ВХОД:\n"
        f"⚠️ НИКОГДА не говори что вход бесплатный/свободный!\n"
        f"Стоимость входа меняется в зависимости от дня/времени/мероприятия.\n"
        f"Если спрашивают про вход → скажи оставить заявку на сайте spb.evgenich.bar или менеджер в чате ответит.\n\n"
        f"# СЕГОДНЯ\n"
        f"{updates_string}\n\n"
    )
