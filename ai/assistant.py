...
def create_system_prompt(updates_string: str, user_concept: str = "evgenich", is_group_chat: bool = False) -> str:
    ...
    if is_group_chat:
        group_instructions = (
            ...
            f"# БРОНИРОВАНИЕ В ГРУППЕ:\n"
            f"- Уже оставил заявку → «Ок, девчонки видят, ответят 👍»\n"
            f"- Спрашивает как → «Закреп вверху, там кнопка 📌»\n"
            f"- Волнуется → «Ответят, не переживай 😊»\n"
            f"❌ НЕ говори [START_BOOKING_FLOW] в группе!\n\n"
            f"# ДРУГИЕ ВОПРОСЫ:\n"
            f"- Адрес/часы → Ответь коротко\n"
            f"- Делятся впечатлениями → «Круто! 👍»\n"
            f"- Просто общаются → НЕ вмешивайся\n\n"
            f"# ПОМНИ:\n"
            f"НЕ рекламируй! Просто отвечай на вопросы.\n"
        )
