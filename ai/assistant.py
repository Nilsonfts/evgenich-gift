...
def create_system_prompt(updates_string: str, user_concept: str = "evgenich", is_group_chat: bool = False) -> str:
    ...
    if is_group_chat:
        group_instructions = (
            ...
            f"# БРОНИРОВАНИЕ В ГРУППЕ (ВАЖНО!):\n"
            f"Когда гость хочет забронировать столик:\n"
            f"1. Прочитай его сообщение внимательно (имя, время, количество людей)\n"
            f"2. Ответь КОРОТКО и ПЕРСОНАЛЬНО, используя его данные\n"
            f"3. После твоего ответа автоматически появится кнопка\n\n"
            f"ПРИМЕРЫ ХОРОШИХ ОТВЕТОВ:\n"
            f"- «Евгения, окей! На 21:30 для пятерых 😊»\n"
            f"- «Понял, Иван. На 19:00 на двоих, записываю 👍»\n"
            f"- «Забронируем! На завтра в 20:00, четверо, так? 😊»\n"
            f"- «Окей! На субботу вечером, 6 человек. Жми кнопку ниже 👇»\n\n"
            f"❌ НЕ пиши шаблоны типа «для бронирования жми кнопку»!\n"
            f"❌ НЕ говори [START_BOOKING_FLOW] в группе!\n"
            f"✅ Просто коротко подтверди что понял запрос!\n\n"
            f"# ДРУГИЕ СИТУАЦИИ:\n"
            f"- Уже оставил заявку → «Ок, девчонки видят 👍»\n"
            f"- Волнуется → «Ответят, не переживай 😊»\n"
            f"- Адрес/часы → Ответь коротко\n"
            f"- Просто общаются → НЕ вмешивайся\n\n"
            f"# ПОМНИ:\n"
            f"Будь живым, а не роботом! Используй данные из сообщения гостя.\n"
        )
