...
def register_handlers(bot):
    ...
    def handle_friend_command(message: types.Message):
        """
        Генерирует и отправляет пользователю его персональную реферальную ссылку в виде кнопки.
        """
        if user_booking_data.get(message.from_user.id):
        ...
        try:
            ...
            bot.send_message(user_id, text_to_send, reply_markup=keyboard)
            
            # Отправляем отдельное сообщение с условиями, чтобы оно не мешалось
            text_after = (
