...
def register_handlers(bot):
    ...
    def handle_friend_command(message: types.Message):
        """
        Генерирует и отправляет пользователю инструкцию для приглашения друга.
        """
        if user_booking_data.get(message.from_user.id):
        ...
        user_id = message.from_user.id
        logging.info(f"Пользователь {user_id} запросил реферальную инструкцию.")
        
        try:
            bot_username = bot.get_me().username
            if not bot_username:
                logging.error("ОШИБКА: У бота не установлен публичный username (@имя_бота).")
                bot.send_message(user_id, "Не удалось создать инструкцию. Администратору бота необходимо установить для него публичное имя пользователя (username) в @BotFather.")
                return

            # Создаем инструкцию вместо прямой ссылки
            referral_code = f"/start ref_{user_id}"
            instruction_text = (
                "💪 Решил перевыполнить план, товарищ? Правильно!\n\n"
                "Иногда Telegram не дает ботам отправлять ссылки. Чтобы я засчитал твоего друга, попроси его сделать следующее:\n\n"
                f"1. Найти меня в поиске Telegram по имени: `@{bot_username}`\n"
                "2. Отправить мне этот код-приглашение (можно просто нажать на него, чтобы скопировать):\n\n"
                f"`{referral_code}`\n\n"
                "Как только он это сделает, я засчитаю его как твоего товарища! ✊"
            )
            
            bot.send_message(user_id, instruction_text, parse_mode="Markdown")

        except Exception as e:
            logging.error(f"Критическая ошибка при создании реферальной инструкции для {user_id}: {e}", exc_info=True)
            bot.send_message(user_id, "Что-то пошло не так при создании инструкции. Попробуйте еще раз.")
